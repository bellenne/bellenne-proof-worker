...
В папке заказа Worker рассматривает непосредственные каталоги `1`, `2`, `3`
или `1 Шумакова`, `2 Иванов` как последовательные ревизии работы. После положительного
номера допускается подпись, отделённая пробелом. Внутри каждой ревизии он
...
Перед проверкой и обработкой Worker потоково копирует найденный исходник в
`WORKER_DATA_PATH/jobs/.../source-cache` (в Docker — локальный volume `/data`).
Проверка, создание миниатюры, анализ и рендер используют локальную копию.
Повторное использование того же исходника внутри задания не требует новой загрузки,
если его размер и время изменения не поменялись. После обработки задания копии
удаляются, в том числе при ошибке; после аварийного завершения остатки удаляются
при повторном запуске обработки той же попытки. На локальном диске должно хватать
места для исходников задания и результатов. SMB используется для поиска,
копирования исходников и записи готовых файлов.
Логи `COPYING_SOURCE`, `SOURCE_COPY_PROGRESS` и `SOURCE_COPIED` показывают начало,
объём скопированных данных (раз в 10 секунд при поступлении данных) и время копирования.

После рендера Worker создаёт в папке заказа следующую числовую ревизию. Если уже
существуют `1 Шумакова`, `2`, `3 Иванов`, результат появится в такой структуре:
...
Связка `Макет N` должна находиться только в одной папке ревизии. Номера макетов
...
встретилась в двух папках ревизий, структура считается неоднозначной и Job
