...
существуют `1`, `2`, `3`, результат появится как
`4/ЦП Макет N 60х30.jpg`. Размер в имени берётся из preset и указывается в
сантиметрах. Повтор того же `job_id + attempt` использует уже созданный файл и
не создаёт ещё одну ревизию.
