...
обнаруживает Worker.
Готовый JPEG записывается в новую непосредственную числовую ревизию папки заказа:
`max(существующие ревизии) + 1/ЦП Макет N 60х30.jpg`. В metadata результата
Worker передаёт `published_revision`, `published_filename` и `published_path`.
